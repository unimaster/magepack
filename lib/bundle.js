...
module.exports = async (bundlingConfig, localesGlobPattern) => {
    console.log(bundlingConfig.config);

    const bundlingConfigRealPath = path.resolve(bundlingConfig.config);



    logger.info(`Using bundling config from "${bundlingConfigRealPath}".`);

    const bundlingConfigFile = require(bundlingConfigRealPath);
    const localesPaths = getLocales(localesGlobPattern, bundlingConfig.theme);

    const isMinifyOn = checkMinifyOn(localesPaths);
    localesPaths.forEach((localePath) => {
        logger.info(`Creating bundles for "${localePath}".`);

        bundlingConfigFile.forEach((bundle) => {
            ...
            if (bundlePath === undefined) {
                console.log(localePath, bundleName);
                throw new Error('Bundle path is undefined');
            }

            if (bundleContents === undefined) {
                console.log(bundlePath);
                throw new Error('Bundle contents is undefined');
            }
