...
const checkout = async (browserContext, generationConfig) => {
    ...
    // Log the error outside of page.evaluate
    ...
    await page.waitForResponse(
        (response) =>
            response.url().includes('/checkout/cart/add/uenc') &&
            response.status() === 200
    );
    ...
    logger.log(
        chalk.blueBright(`⇒`) +
            ...
            chalk.blueBright(`${baseUrl}checkout`) +
            `"`
    );
    await page
        .goto(`${baseUrl}checkout`, { waitUntil: 'networkidle0' })
        .catch((err) => console.log(err));
