...
let excludedModules = require('./excludedModules');
let excludesAppended = false;
/**
 * Collects all defined RequireJS modules on a given page.
 */
module.exports = async (page, exclMods = '') => {
    logger.debug(`Start collect modules`);
    if (exclMods && !excludesAppended) {
        excludedModules = excludedModules.concat(exclMods.split(','));
        excludesAppended = true;
    }

    logger.debug(`Wait to make sure RequireJS is loaded.`);
    /**
    ...
    logger.debug(`Use Magento's rjsResolver to wait for all modules to load.`);
    /**
    ...
    logger.debug(`Wait for browser to be idle for a good measure.`);
    /**
    ...
    // logger.info(`Wait another 5s for a good measure.`);
    // /**
